#!/usr/bin/env bash
# Regenerates the five probe files the golden fingerprint tests read.
#
...
#
# Re-running this reproduces `probe.mp3`, `probe.flac` and `probe.m4a` byte for byte.
#  It does not reproduce `probe.ogg` or `probe.opus`: an Ogg stream carries a random
#  serial number, so a handful of bytes change per run. The decoded audio does not,
#  and neither does the fingerprint, so `probe.flac.uri`, the one golden left,
#  survives a regeneration. Checked on ffmpeg 8.0.1; another build may re-encode
#  differently, and then the golden has to be rewritten alongside the audio.
set -euo pipefail
